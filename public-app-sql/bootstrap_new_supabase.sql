...
CREATE POLICY "Users can insert own logs" ON public.usage_logs
  FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can delete own logs" ON public.usage_logs
  FOR DELETE USING (auth.uid() = user_id);


-- ============================================
-- 8. blog_history 테이블 (pgvector 유사도 검사)
-- ============================================
...
-- IVFFlat 유사도 검색 인덱스 (데이터 100건 이상일 때 효과적)
-- 초기에는 데이터가 없으므로 주석 처리. 데이터 쌓이면 실행:
