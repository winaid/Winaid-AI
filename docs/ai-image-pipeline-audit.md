...
| 3 | ✅ resolved — **OPENAI_IMAGE_MODEL snapshot pin 문서화 + invariant 가드** (Round 2 commit). 양 앱 `.env.example` 안내 + README 환경변수 표 신규 행 (`gpt-image-2-2026-04-21` 권장). `fixedPolicyInvariant.test.ts` 가 `.env.example` 와 README 의 키·snapshot 패턴 존재를 invariant 로 강제 | 중간 | 낮음 |
...
| 5 | ✅ resolved (인프라) — **gemini 모델 deprecation 추적** alias map + runtime warning (Round 2 commit). 신규 `packages/blog-core/src/llm/models.ts` — `resolveModel(id)` 가 alias 자동 적용 + 1회 dedup warn. callee (claude.ts / gemini.ts / next-app geminiDirect.ts / `/api/gemini` route.ts) 4 곳 wiring → 호출지 30+ 위치 변경 0. GA 전환 발견 시 운영자가 `DEPRECATED_MODELS` set + `MODEL_ALIASES` map 만 업데이트 → 자동 마이그레이션. **호출지 점진 적용** (직접 `resolveModel` import) 은 별도 follow-up | 중간 | 중간 |
