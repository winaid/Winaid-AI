...
(async () => {
  ...
  await test('route: subscriptions POST hospital_name / our_domains / channels 검증', () => {
    const p = resolve(REPO_ROOT, 'public-app/app/api/geo/alerts/subscriptions/route.ts');
  ...
  await test('route: evaluate hospital_name 필수 + maxDuration 30', () => {
    const p = resolve(REPO_ROOT, 'public-app/app/api/geo/alerts/evaluate/route.ts');
    const src = readFileSync(p, 'utf-8');
    assert.ok(/hospital_name 필수/.test(src));
    assert.ok(/export const maxDuration = 30/.test(src));
  });

  await test('route: public-app — 게스트 차단 + admin_session bypass + 로그인 필수', () => {
    const p1 = resolve(REPO_ROOT, 'public-app/app/api/geo/alerts/subscriptions/route.ts');
    const p2 = resolve(REPO_ROOT, 'public-app/app/api/geo/alerts/evaluate/route.ts');
    for (const p of [p1, p2]) {
      const src = readFileSync(p, 'utf-8');
      assert.ok(/admin_session=/.test(src), `${p}: admin_session bypass 누락`);
      assert.ok(/isAuthenticatedByCookie/.test(src), `${p}: 게스트 차단 누락`);
      assert.ok(/로그인이 필요/.test(src), `${p}: 401 메시지 누락`);
    }
  });
