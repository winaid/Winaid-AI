...
// eslint-disable-next-line no-console
console.log('\n>>> competitorWatch.test.ts — public-app');
...
(async () => {
  ...
  await test('route: detect — hospital_name 필수 + maxDuration 90', () => {
    const p = resolve(REPO_ROOT, 'public-app/app/api/geo/competitor/detect/route.ts');
  ...
  await test('route: list — GET/POST/DELETE 모두 export + hospital_name 검증', () => {
    const p = resolve(REPO_ROOT, 'public-app/app/api/geo/competitor/list/route.ts');
    const src = readFileSync(p, 'utf-8');
    assert.ok(/export const GET/.test(src));
    assert.ok(/export const POST/.test(src));
    assert.ok(/export const DELETE/.test(src));
  ...
  await test('route: respond — competitor_content_id 필수 + prefillUrl 생성', () => {
    const p = resolve(REPO_ROOT, 'public-app/app/api/geo/competitor/respond/route.ts');
    const src = readFileSync(p, 'utf-8');
  ...
  await test('route: public-app — 게스트 차단 + admin_session bypass 3 라우트 모두', () => {
    for (const f of ['detect', 'list', 'respond']) {
      const p = resolve(REPO_ROOT, `public-app/app/api/geo/competitor/${f}/route.ts`);
      const src = readFileSync(p, 'utf-8');
      assert.ok(/admin_session=/.test(src), `${f}: admin_session 누락`);
      assert.ok(/isAuthenticatedByCookie/.test(src), `${f}: 게스트 차단 누락`);
      assert.ok(/로그인이 필요/.test(src), `${f}: 401 메시지 누락`);
    }
  });
