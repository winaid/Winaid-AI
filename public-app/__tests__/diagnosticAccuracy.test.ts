...
const SCORING_SRC = readFileSync(resolve(REPO_ROOT, 'public-app/lib/diagnostic/scoring.ts'), 'utf-8');
...
test('Phase A #5: 외부 채널 hostname suffix — naver 정확 매칭', () => {
  ...
  // false positive 차단 (substring naver 가 host 가 아닌 경우)
...
test('Phase A #5: 외부 채널 hostname suffix — youtube / instagram', () => {
  ...
  assert.ok(matchesCh('https://www.youtube.com/@channel', yt));   // www. 서브도메인 OK (suffix)
...
test('Phase A #5: 외부 채널 — link text 매칭 회귀 차단', () => {
  // 옛 externalLinksHaystack 가 href + text 결합 후 substring 매칭했으나
  // 새 hasChannelMatch 는 href 만 hostname 추출. text 무시 invariant.
  assert.ok(
...
test('Phase A #1: scoring.ts — has_doctor_page 3단계 구조 (path AND name)', () => {
  // 새 구현은 hasDoctorPath + hasDoctorName 둘 다 변수로 사용 + 3단계 분기.
  assert.ok(
