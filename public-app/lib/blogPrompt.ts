...
**각 li 항목은 반드시 1~2문장 설명 포함**. 라벨·제목만 나열 금지.
  ✅ <ul><li><strong>잇몸 관리</strong>: 치간 칫솔을 매일 사용하고, 양치 후 혀 표면까지 꼼꼼히 닦아주세요.</li>
       <li><strong>정기 검진</strong>: 6개월에 한 번씩 스케일링과 함께 잇몸 상태를 확인하는 것이 좋습니다.</li></ul>
  ❌ "관리에는 잇몸 관리, 정기 검진, 금연이 있습니다." (평문 나열)
  ❌ "1단계 3D CT 진단 / 2단계 식립 / 3단계 보철" (제목만 나열, 설명 0) ← 교과서 스타일 금지
</scannable_list>
...
2. 3개 이상 나열 → ul/li 리스트. **각 li는 1~2문장 설명 필수** (제목만 나열 금지).
3. 핵심 수치(기간·비율) → strong 강조.
4. ul/li 항목 첫 단어를 strong으로 라벨화.
5. 핵심 키워드는 단락 첫 줄에 등장 (두괄식 강조).
6. 다음 문단 첫 문장이 이전 주제를 자연스럽게 이어받기.
7. 구체 수치 또는 환자 체감 표현("찌릿한", "욱신거리는") 문단당 1개+.
8. **시술·치료 단계 설명은 리스트보다 단락 서술 우선**. 번호+제목만 나열 금지.
   ❌ "1단계 진단 / 2단계 시술 / 3단계 관리" (교과서 목차)
   ✅ 각 단계를 독립 <p>로 풀어서 "먼저 ~합니다. 이 단계에서는 ~." 형태로 서술
</paragraph>
