...
${range.max}자 초과 시 → 가장 약한 문단을 삭제 후 출력. 패딩(같은 말 반복, 원론적 경고) 금지.`;
  ...
  const systemInstruction = [
    ...
    '',
    '[가독성 강화 규칙]',
    '1. 한 <p> 문단은 최대 4문장, 150자 이내. 초과하면 반드시 <p>를 나누세요.',
    '2. 3개 이상 나열(증상, 방법, 주의사항 등)할 때는 반드시 <ul><li> 리스트 사용.',
    '   예시: "주의사항은 세 가지입니다. 첫째..." ❌ → <ul><li>첫째...</li><li>둘째...</li></ul> ✅',
    '3. 핵심 수치(기간·비율·크기)와 결론 문장은 <strong>으로 강조.',
    '   예시: "약 <strong>2~3개월</strong> 정도 소요됩니다"',
    '4. 각 소제목(h3) 아래 첫 문장은 질문형 또는 상황 묘사형으로 시작.',
    '   ✅ "칫솔질만으로 충분할까요?"',
    '   ✅ "아침에 양치할 때 피가 묻어나온 경험이 있으신가요?"',
    '   ❌ "잇몸 질환이란 잇몸 조직에 염증이..."',
    '5. 소제목 사이에 최소 2개, 최대 4개 문단. 1개 문단 섹션 금지.',
    '6. 모든 소제목(h3) 텍스트에 키워드 1회 포함 권장 (자연스럽게).',
    '',
    '[독자 말 걸기 — 허용되는 형식]',
...
export function buildSectionRegeneratePrompt(input: SectionRegenerateInput): {
  ...
  const systemInstruction = [
    ...
    '',
    '[가독성 강화 규칙]',
    '1. 한 <p> 문단은 최대 4문장, 150자 이내. 초과 시 <p> 분리.',
    '2. 3개 이상 나열 → 반드시 <ul><li> 리스트. 본문 속 "첫째... 둘째..." 텍스트 나열 금지.',
    '3. 핵심 수치·결론 → <strong> 강조.',
    '4. 소제목(h3) 아래 첫 문장 = 질문형 또는 상황 묘사형. 정의형("~이란") 금지.',
    '5. 소제목 사이 최소 2개, 최대 4개 문단.',
    '6. 소제목(h3) 텍스트에 키워드 1회 포함 권장.',
    '',
    categoryGuide,
    '',
    '[출력 규칙]',
...
export function buildBlogPromptV3(
  ...
  // 학습 말투 경로 오버라이드 — BASE_PERSONA_V3 의 [절대 규칙](인사 고정 형식) + [출력 형식](태그 화이트리스트)
  // 을 non-cacheable variable 에서 약화. 캐시 prefix 는 그대로 두고 이 블록만 동적으로 주입.
  // 트리거: stylePromptText(UI 수동 학습) 또는 opts.hospitalStyleBlock(관리자 DB 학습) 둘 중 하나라도 있으면.
  const hsbText = typeof opts.hospitalStyleBlock === 'string'
    ? opts.hospitalStyleBlock
    : opts.hospitalStyleBlock?.systemBlock ?? '';
  const hasStyleOverride = !!(req.stylePromptText && req.stylePromptText.trim())
    || !!(hsbText && hsbText.trim());
  const styleOverrideBlock = hasStyleOverride ? [
    '[학습 말투 경로 — 인사 및 단락 간격 오버라이드]',
    '1. BASE_PERSONA 의 "[절대 규칙]" (첫 <p> 를 "안녕하세요. {수식구} {병원명} {직책}입니다." 형식으로 강제) 를 **적용하지 않는다**.',
    '   - 학습 말투 블록의 [원문 단락 샘플] 첫 단락이 인사로 시작하면 그 인사 스타일/길이를 그대로 재현. 병원명·직책이 원문에 없으면 강제로 넣지 마라.',
    '   - 학습 말투 블록의 [원문 단락 샘플] 첫 단락이 인사 없이 본론으로 시작하면 인사 없이 바로 본론으로 시작한다.',
    '   - 표준 인사 "안녕하세요. {수식구} {병원명} {직책}입니다." 는 이 경우 적용하지 않는다.',
    '2. BASE_PERSONA 의 "[출력 형식] 허용 태그" 에 추가로 **빈 <p></p> 한 개 삽입을 허용한다**. 학습 말투 블록의 "원문의 빈 줄(\\n\\n) 이 단락을 분리하는 자리" 에 한해 사용.',
    '   - 빈 <p></p> 연속 2개 이상 금지. 일반 단락 구분은 <p>문장</p><p>문장</p> 기본.',
  ].join('\n') : '';
...
export function buildBlogReviewPrompt(
  draftHtml: string,
  ctx: { category?: string; hospitalName?: string; ruleFilterViolations?: string[]; stylePromptText?: string; hospitalStyleBlock?: string } = {},
  ...
  // 학습 말투 경로 오버라이드 — REVIEWER_PERSONA 의 [인사 문장 보존 및 복원] 블록을 무효화.
  // 캐시 prefix 는 그대로, non-cacheable variable 에서 동적으로 override.
  // 트리거: stylePromptText(UI 수동 학습) 또는 hospitalStyleBlock(관리자 DB 학습) 둘 중 하나라도 있으면.
  const hasReviewOverride = !!(ctx.stylePromptText && ctx.stylePromptText.trim())
    || !!(ctx.hospitalStyleBlock && ctx.hospitalStyleBlock.trim());
  const styleOverride = hasReviewOverride
    ? '[학습 말투 경로 — 인사 복원 건너뛰기]\n이 요청은 학습 말투 경로다. REVIEWER_PERSONA 의 "[인사 문장 보존 및 복원]" 블록 전체를 건너뛰고, 초안의 인사 유무·형식을 있는 그대로 존중한다. 인사가 없어도 MISSING 으로 판정하지 말고, 쪼개져 있어도 FRAGMENTED/TRUNCATED 로 판정하지 마라.\n'
    : '';
