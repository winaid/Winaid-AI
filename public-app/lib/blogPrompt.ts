...
<scannable_list label="리스트로 답변 분리">
3개 이상 항목 나열 시 ul/li. 각 항목 시작에 strong 라벨.
  ✅ <ul><li><strong>잇몸 관리</strong>: 치간 칫솔 사용</li>...</ul>
  ❌ "관리에는 잇몸 관리, 정기 검진, 금연이 있습니다." (평문)
</scannable_list>
...
2. 문단당 최대 4문장, 150자 이내. 한 문장은 최대 60자 (모바일 2줄 기준).
...
11. 핵심 키워드는 단락 첫 줄에 등장 (두괄식 강조). ul/li 항목은 strong 라벨로 시작.
12. 도입부 200자 안에 "독자/주제/얻을 가치" 3요소 전달 (모바일 첫 화면 대응).
</writing_style>
...
각 소제목 아래 문단 2~4개. 목표 글자수 ±20%.

<length_guarantee>
글을 다 쓴 후 총 글자수(태그 제외, 텍스트만)를 반드시 확인하세요:
- 목표 × 0.8 미만이면: 가장 내용이 얇은 섹션에 구체 사례·수치·환자 체감 표현을 추가
- 목표 × 1.2 초과면: 반복되는 문장이나 패딩("이처럼", "~한 만큼")을 삭제

글자수가 부족할 때 같은 말 반복으로 채우지 마세요 — 새로운 정보를 추가하세요:
  ✅ 관련 주의사항 추가, 자주 묻는 질문 1개 추가, 구체 사례 보강
  ❌ "이처럼 관리가 중요합니다. 관리의 중요성은 아무리 강조해도..." (패딩)
</length_guarantee>

충돌 시: 글자수 > 소제목 수. 글자수 맞추기 위해 소제목 추가/삭제 가능.
</volume_rules>
...
□ 분량: 태그 제외 텍스트만 세어서 목표 × 0.8 이상인가? 부족하면 얇은 섹션에 새 정보 추가.
...
□ 훅: 도입부 첫 문장이 5유형(질문/상황/통계/대비/스토리) 중 하나에 해당하는가? 이전 글과 같은 유형 아닌가?
□ 마무리: 마지막 문단이 5유형(상담권유/예방강조/자기점검/가족안내/다음단계) 중 하나에 해당하는가? "궁금한 점은~" 일변도가 아닌가?
□ 모바일: 60자 초과 문장이 있는가? → 분리
...
10. 소제목 아래 첫 문장 = 짧은 직답 (Yes/No/숫자/핵심). 이후 확장 서술.
...
14. intro 섹션이면 variable의 훅 유형 힌트를 따르세요. 힌트 없으면 주제에 맞게 5유형 중 자유 선택.
15. outro 섹션이면 variable의 마무리 유형 힌트를 따르세요. 시술 유인 금지.
16. 한 문장 최대 60자 (모바일 가독성). 초과 시 분리.
17. ul/li 항목 첫 단어를 strong으로 라벨화.
18. variable의 charTarget을 확인하고 ±15% 범위로 작성하세요.
    부족하면 구체 사례·수치를 추가. 초과하면 반복·패딩을 삭제.
    절대 같은 말 반복으로 채우지 마세요.
</writing_style>
