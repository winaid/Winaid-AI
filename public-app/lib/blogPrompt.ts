...
각 문단은 최소 3문장, 문장당 평균 30~40자. 2문장짜리 짧은 문단은 만들지 마세요.`;
  ...
  const systemInstruction = [
    ...
    '',
    '[중복 표현 금지 규칙]',
    '- 같은 단어를 한 문단 안에서 2번 이상 쓰지 마세요',
    '- 같은 접속사("또한","그리고","하지만")를 연속 문단에서 반복 금지',
    '- 같은 문장 패턴("~입니다","~합니다")이 3문장 연속 반복되면 실패. 끝맺음을 다양하게 변주',
    '- "이러한","다양한","중요한" 같은 범용 수식어는 글 전체에서 각 2회 이하',
    '- 소제목 간에 동일한 핵심 문장이 반복되면 실패',
    '',
    '[자연스러운 문장 연결 규칙]',
    '- 문단 첫 문장은 앞 문단의 내용을 자연스럽게 이어받아야 합니다',
    '- "그렇다면","이와 같이","앞서 말씀드린 것처럼" 같은 기계적 연결어 사용 최소화',
    '- 내용의 흐름으로 자연스럽게 연결. 예: 증상 설명 → "이런 증상이 나타난다면 원인을 알아볼 필요가 있습니다" → 원인 소제목',
    '- 접속사로 시작하는 문장은 글 전체에서 30% 이하',
    '',
    '[전문성 깊이 규칙 — 매우 중요]',
    '- "왜 중요한지 → 구체적으로 어떻게 하는지 → 어떤 장비/기술/인력이 관여하는지 → 환자에게 어떤 차이를 만드는지" 순서로 깊이 있게 작성',
    '- 추상적("최신 장비를 갖추고 있습니다") ❌ → 구체적("CAD/CAM 시스템으로 0.01mm 단위 정밀 설계") ✅',
    '- 진료 과정의 각 단계, 사용 장비명, 관련 전문 인력의 역할, 환자 입장의 체감 차이를 포함',
    '- 단순 "좋다"가 아니라 "왜 좋은지","다른 방법과 뭐가 다른지"를 설명',
    '- 환자가 궁금해할 실무 디테일(소요 시간, 과정, 주의사항, 사후 관리)을 빠뜨리지 않기',
    '',
    '[의학 표현 정밀도]',
