...
⚠️ 인용은 실제 발화가 아닌 기사 구성용 가공 인용입니다. 자연스럽고 의학적으로 정확하게 작성하되 실존 인물의 실제 발언을 인용하는 것처럼 보이지 않도록 주의하세요.

[통계/수치 규칙 — 할루시네이션 방지]
- Google Search로 확인된 수치만 사용. 확인 불가능하면 수치 대신 일반적 서술 사용
  ❌ "건강보험심사평가원에 따르면 23만 건이 시행되었다" (검증 불가 수치)
  ✅ "최근 몇 년간 해당 시술 건수가 증가 추세를 보이고 있다" (일반 서술)
- 구체적 출처를 밝힐 때는 기관명만 (URL 금지). 기관이 실제로 해당 데이터를 발표했는지 확인 불가하면 "전문가들에 따르면" 사용
- 확실하지 않은 수치는 범위로 ("약 3~6개월", "수만 건 이상")
...
${PRESS_CATEGORY_GUIDES[req.category || ''] ? `\n[${req.category} 전문 용어 — 기사에 정확한 명칭 사용]\n${PRESS_CATEGORY_GUIDES[req.category || '']}` : ''}`;
  ...
  if (req.pressType === 'health_tips') {
    promptParts.push(
      ...
      '[건강 조언 기사 특별 규칙]',
      '- 의학 정보는 관찰형 서술만 사용:',
      '  ✅ "~로 보고되고 있다", "~로 알려져 있다", "전문가들은 ~로 보고 있다"',
      '  ❌ "효과가 좋다", "도움이 된다", "치료가 가능하다" (단정형 금지)',
      '- 예방/관리 방법은 구체적 행동으로:',
      '  ❌ "구강 관리가 중요합니다" (추상적)',
      '  ✅ "하루 2회 양치질과 6개월 간격 스케일링이 권장된다" (구체적)',
      '- 수치가 있으면 반드시 포함 (발생률, 연령대, 빈도, 기간 등)',
      '- 출처 표현: "대한○○학회에 따르면", "건강보험심사평가원 통계에 따르면" (신뢰 기관만)',
      '- "개인차가 있을 수 있다", "정확한 진단은 전문의 상담이 필요하다" 류 문구 필수 포함',
    );
  }
