...
⛔ h2 부제 태그 출력 금지!`;
  ...
  if (maxLength <= 800) {
    promptParts.push(
      ``,
      `[⚠️ 기사 분량: 짧은 기사 (단신/속보형)]`,
      `- 목표 글자 수: 공백 제외 700~800자`,
      `- 본문 단락: 정확히 3~4개 <p> 태그만 사용`,
      `- 전문의 인용: 1회만 (짧은 기사라 1회로 충분)`,
      `- 구성: 도입 1문단 → 핵심 정보 1~2문단 → 전문의 코멘트 1문단`,
      `- 짧고 간결하게! 부연 설명이나 배경 설명 최소화`,
      `- 한 문단 최대 3문장 이내`,
    );
    ...
    promptParts.push(
      ...
      `- 구성: 도입 1문단 → 상황 설명 1~2문단 → 의학적 정보 2문단 → 전문의 코멘트 1~2문단 → 마무리 1문단`,
      `- 일반적인 보도 기사 분량으로 적당한 깊이`,
    );
  } else {
    promptParts.push(
      ``,
      ...
      `- 본문 단락: 9~12개 <p> 태그 사용`,
      `- 전문의 인용: 3회 이상 (다양한 각도에서 심층 인용)`,
      `- 구성: 도입 1~2문단 → 사회적 배경/트렌드 2문단 → 의학적 원인/메커니즘 2~3문단 → 치료/관리 방법 2문단 → 전문의 심층 코멘트 2문단 → 마무리 1문단`,
      `- 깊이 있는 의학 정보와 사회적 맥락을 풍부하게 서술`,
      `- 통계나 연구 결과를 Google Search로 찾아 포함`,
      `- 반드시 1600자 이상 작성할 것! 짧게 쓰지 마!`,
    );
  }
