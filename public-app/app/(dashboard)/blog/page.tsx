...
function BlogForm() {
  ...
  // GEO-12: from=geo_funnel 시 추가 prefill 은 이미 위에서 parseCampaignPrefill 호출
  const funnelCategory = parseFunnelParams(searchParams).category;
  const initialCategoryRaw = geoPrefill.category || funnelCategory;
  const initialCategory = (initialCategoryRaw && (Object.values(ContentCategory) as string[]).includes(initialCategoryRaw))
...
JSON 형식으로 응답해주세요.`;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 이미지 즉시 병렬 생성 (review 와 동시에) + Supabase Storage 즉시 업로드
      // (base64 inline 으로 본문에 들어가면 generated_posts.content 가 9MB+ 로 폭증 — PR #145 진단)
        ...
        imageResultsPromise = Promise.all(
          imagePrompts.slice(0, imageCount).map((p, i) => {
            ...
            return fetch('/api/image', {
              ...
              .then(async d => {
                const dataUrl = (d?.imageDataUrl as string | undefined) || null;
                if (!dataUrl) return { index, url: null as string | null };
                // Storage 업로드 시도 — 성공 시 public URL, 실패 시 null (이후 generateAndUpload 가 재시도)
            ...
            if (res.ok) {
              ...
              for (const marker of imgMarkers) {
                ...
                const best = pickBestLibraryImage(libraryImages, {
                  ...
                  // → 자연스럽게 remainingMarkers 로 떨어져 AI 생성 fallback.
      ...
      try {
        ...
        console.info(`[BLOG] [V4] Opus 검수 완료 — verdict=${review.verdict}, issues=${issues.length}, summary="${(review.summaryNote || '').slice(0, 80)}"`);
        ...
        if (review.verdict !== 'pass' && issues.length > 0) {
          ...
          if (review.revisedHtml && typeof review.revisedHtml === 'string' && review.revisedHtml.length > 100) {
            ...
            const lossThreshold = Math.min(beforeImgCount, imageCount);
            revisedHtmlPath = lossThreshold > 0 && revisedImgCount < lossThreshold ? 'lost_imgs' : 'ok';
          }
