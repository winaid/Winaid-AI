...
// ── P2-6: humanizeProgress 필터 ──
describe('P2 humanizeProgress 내부 용어 필터', () => {
  it('"Stage A 파이프라인" → 빈 문자열', () => {
    expect(humanizeProgress('Stage A 파이프라인 시작')).toBe('');
  });

  it('"폴리싱 진행 중" → 빈 문자열', () => {
    expect(humanizeProgress('폴리싱 진행 중')).toBe('');
  });

  it('"소제목 2/5 작성" → 빈 문자열', () => {
    expect(humanizeProgress('소제목 2/5 작성 중')).toBe('');
  });

  it('"도입부 작성 중" → 빈 문자열', () => {
    expect(humanizeProgress('도입부 작성 중')).toBe('');
  });

  it('"섹션 3 생성" → 빈 문자열', () => {
    expect(humanizeProgress('섹션 3 생성 중')).toBe('');
  });

  it('"파이프라인 생성 완료" → 빈 문자열', () => {
    expect(humanizeProgress('파이프라인 생성 완료')).toBe('');
  });

  it('"AI 냄새 제거" → 빈 문자열', () => {
    expect(humanizeProgress('AI 냄새 제거 중')).toBe('');
  });

  it('"이미지 3/5장 생성 중 (hero)" → "(hero)" 제거', () => {
    expect(humanizeProgress('이미지 3/5장 생성 중 (hero)')).toBe('이미지 3/5장 생성 중');
  });

  it('"이미지 1/5장 완료 (sub)" → "(sub)" 제거', () => {
    expect(humanizeProgress('이미지 1/5장 완료 (sub)')).toBe('이미지 1/5장 완료');
  });

  it('"보조 비주얼 유지" → 빈 문자열', () => {
    expect(humanizeProgress('⚠️ 대표 이미지 재시도 실패 — 보조 비주얼 유지')).toBe('');
  });

  it('정상 메시지는 그대로', () => {
    expect(humanizeProgress('✅ 이미지 3/5장 완료')).toBe('✅ 이미지 3/5장 완료');
  });

  it('빈 문자열은 빈 문자열', () => {
    expect(humanizeProgress('')).toBe('');
  });
});
