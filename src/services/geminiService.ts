...
`,
    casual: `
[말투: 친근하고 쉬운 톤 (2030 타겟)]
- 20~30대가 읽기 편한 가벼운 문체
- 전문 용어 최소화, 일상어 위주
- 짧은 문장, 호흡이 빠른 리듬
- "~거든요", "~인 거예요" 식의 구어체 허용 (단, 본문 30% 이하)
- 비유는 일상적인 것으로: "충치는 벌레가 파먹는 게 아니라~"
- 치과 예시: "스케일링 아프다는 거 다 옛날 얘기예요"
- 핵심 정보는 정확하되, 톤만 가볍게
`,
    reassuring: `
[말투: 안심시키는 톤 (치과 공포 해소)]
...
`,
    '예비환자(검색유입)': `
[청중: 아직 병원에 안 간 검색 유입자]
- "이거 나도 해당되나?" 궁금한 사람
- 증상 체크리스트, 자가진단 기준 제공
- 병원 방문 시점 판단에 도움 되는 정보 위주
- 비용/기간/통증 수준 등 현실적 궁금증 해소
- 치과 예시: "어금니가 시린데 충치인지 잇몸 문제인지 구분하는 법"
- 마무리에 "~라면 확인이 필요한 시점일 수 있습니다" 식의 열린 결말
`,
    '보호자용(가족걱정)': `
...
`,
    '재방문유도(기존환자)': `
[청중: 이미 한 번 이상 방문한 기존 환자]
- 치료 후 관리, 정기 검진의 중요성 위주
- "치료 후 ~개월 시점에는" 식의 시간 기준 정보
- 재발 방지, 유지 관리, 다음 단계 안내
- 치과 예시: "임플란트 식립 후 3개월 체크가 중요한 이유"
- 이미 신뢰 관계가 있으므로 전문적 톤 허용
`
  };
...
`,
    hygienist: `
[페르소나: 치위생사 시점]
- 환자 옆에서 직접 케어하는 치위생사가 쓴 글
- "스케일링할 때 보면~", "칫솔질 방법을 알려드리면~"
- 실용적 케어 팁, 홈케어 방법 위주
- 의학 이론보다 현장 경험 기반 조언
- "저희가 환자분들께 꼭 말씀드리는 게 있는데요"
- 구강 위생 관리, 올바른 칫솔질, 치실 사용법 등에 강점
- 1인칭 사용 허용 (전체 3~5회 이내)
`,
    storyteller: `
[페르소나: 환자 경험 스토리텔러]
- 3인칭 내러티브 ("A씨는~", "30대 직장인 B씨는~")
- 실제 있을 법한 환자 시나리오를 구성하여 정보 전달
- 이야기 속에 의학 정보를 자연스럽게 녹임
- 기승전결 구조: 증상 발견 → 고민 → 정보 탐색 → 이해
- 치과 예시: "오래 미루다 검진 받은 직장인이 의외로 가벼운 치료만 필요했던 이야기"
- 실존 인물처럼 느껴지되, 실제 환자라고 명시하지 않음 (의료광고법 준수)
- 스토리 안에서 자연스럽게 정보 전달 ("그때 알게 된 건~")
`
  };
