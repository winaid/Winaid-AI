...
import { generateBlogImage, generateImageQueue, isDemoSafeMode, updateSessionFinalPayload } from "./image/imageOrchestrator";
import { classifySceneType, buildScenePrompt } from "./image/imageRouter";
import type { ImageQueueItem } from "./image/imageTypes";
import { analyzeStyleReferenceImage } from "./image/imageEditService";
import { generateSingleImage } from "./image/cardNewsImageService";
...
// FAQ/섹션재생성 → faqService.ts로 이동 — re-export (역호환)
...
// → src/services/image/imageRouter.ts로 이동됨
// classifySceneType, buildScenePrompt는 상단 import에서 가져옴
...
export const generateFullPost = async (request: GenerationRequest, onProgress?: (msg: string) => void): Promise<GeneratedContent> => {
  // 실제 오케스트레이션은 generateContentJob.ts의 orchestrateFullPost()에서 수행
