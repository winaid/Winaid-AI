...
import { callGeminiWithSearch } from "./searchService";
import { evaluateContentQuality } from "./contentQualityService";

// search 함수 → searchService.ts, quality → contentQualityService.ts로 단일화됨
...
`;
  ...
  const searchResult = await callGeminiWithSearch(pressPrompt, { responseFormat: "text/plain" });
  let pressContent = typeof searchResult === 'string' ? searchResult : (searchResult?.text || JSON.stringify(searchResult) || '');
...
`;
  ...
  return {
    ...
    factCheck: evaluateContentQuality(finalHtml),
    postType: 'press_release',
