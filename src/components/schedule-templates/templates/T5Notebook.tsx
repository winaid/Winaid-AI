 ...
 *
 * 리서치 근거:
 * - 똑닥(DDoCDoc) 병원 안내문 템플릿: 한국에서 가장 널리 쓰이는 표준 병원 공지 포맷 (blog.hospital.ddocdoc.com)
 * - Doc4U 휴진안내문 자동생성기: 장식 제로, 정보만 전달하는 구조 (doc4u.co.kr)
 * - Pinterest "심플한 안내문 디자인" (929,000+ 핀): 상단 컬러 바 + 좌측 정렬 + 번호 목록 패턴
 * - 2026 트렌드 "의도적 미니멀리즘": 핵심 메시지에 집중하는 절제된 디자인 (design_trends_research)
 * - 공지사항 최우선 원칙 = 가독성 (배경 밝게, 텍스트 고대비)
 *
 * 시각 요소 매핑:
 * - 상단 블루 바 4px → 똑닥/미리캔버스 공통 "컬러 헤더 바" 패턴
 * - 좌측 정렬 클리닉명+제목 → 문서형 "좌→우 읽기 흐름" (가장 안정적인 레이아웃)
 * - 연한 블루 헤더 배경 #E3F0FC → 문서 표 헤더 구분 (엑셀/공문 표 관습)
 * - 번호 목록 안내사항 → 체크리스트형 공지 패턴 (넘버링 > 불릿 가독성 연구)
 * - 하단 푸터 라인+클리닉명 → 공문 결문(끝맺음) 구조
 */
