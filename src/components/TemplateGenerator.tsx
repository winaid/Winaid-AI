...
} from '../features/template/config/templatePresets';
import {
  loadStyleHistory,
...
export default function TemplateGenerator({ onSwitchToFree }: { onSwitchToFree?: () => void }) {
  ...
  // 채용/공고 — 타입/상수는 templatePresets에서 import
  ...
  // 주의사항 — CAUTION_DEFAULTS는 templatePresets에서 import
  ...
  // 결과 + 생성 오케스트레이션 (훅)
  const gen = useTemplateGeneration();
  const { generating, generatingStep, generatingPage, resultImages, currentPage, error,
    showRegenMenu, regenPrompt, showRegenPromptInput,
    setCurrentPage, setError, setGenerating, setResultImages,
    setShowRegenMenu, setRegenPrompt, setShowRegenPromptInput,
    resetResults,
  } = gen;
  ...
  useEffect(() => { setDayMarks(new Map()); setShortenedHours(new Map()); setVacationReasons(new Map()); resetResults(); }, [month, year]);
  useEffect(() => { resetResults(); setSelectedCatTemplate(null); }, [category]);

  // 명절 자동 기본값 — HOLIDAY_DEFAULTS는 templatePresets에서 import
  useEffect(() => {
  ...
  // ── templateData 조립 (카테고리별 분기) ──
  const buildTemplateData = (): Record<string, any> => {
    ...
    if (category === 'schedule') return { month, year, title: scheduleTitle || `${month}월 휴진 안내`, closedDays: closed, shortenedDays: shortened.length > 0 ? shortened : undefined, vacationDays: vacation.length > 0 ? vacation : undefined, notices: notices.split('\n').filter(Boolean), layout: scheduleLayout, colorTheme: calendarTheme };
    if (category === 'event') return { title: evTitle, subtitle: evSubtitle || undefined, price: evPrice || undefined, originalPrice: evOrigPrice || undefined, discount: evDiscount || autoDiscount || undefined, period: evPeriod || undefined, description: evDesc || undefined };
    if (category === 'doctor') return { doctorName: docName, specialty: docSpecialty, career: docCareer.split('\n').filter(Boolean), greeting: docGreeting || undefined, doctorPhotoBase64: docPhotoBase64 || undefined };
    if (category === 'notice') return { title: noticeTitle, content: noticeContent.split('\n').filter(Boolean), effectiveDate: noticeDate || undefined };
    if (category === 'hiring') return { pageData: hiringPageData.slice(0, hiringPageCount).map(p => ({ type: p.type, content: p.content })), hospitalPhotos: hiringPhotos.length > 0 ? hiringPhotos : undefined };
    if (category === 'caution') return { type: cautionType, title: cautionTitle || `${cautionType} 주의사항`, items: cautionItems.split('\n').filter(Boolean), emergency: cautionEmergency || undefined };
    if (category === 'pricing') return { title: pricingTitle || '비급여 진료비 안내', items: pricingItems.split('\n').filter(Boolean), notice: pricingNotice || undefined };
    return { holiday: greetHoliday, greeting: greetMsg, closurePeriod: greetClosure || undefined };
  };
  ...
  const handleGenerate = async (regenExtra?: string) => {
    await gen.handleGenerate({
      ...
      totalPages: category === 'hiring' ? hiringPageCount : 1,
    }, regenExtra);
    setStyleHistory(loadStyleHistory());
  };
  ...
  const handleDownload = (pageIndex?: number) => {
    gen.handleDownload(category, hospitalName, month, pageIndex);
  };
  ...
  const ph = CATEGORY_PLACEHOLDERS;
