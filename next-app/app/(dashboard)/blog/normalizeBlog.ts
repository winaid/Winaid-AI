...
export function normalizeBlogStructure(
  ...
  //
  //     매칭 기준: <p> 안 텍스트가 다음 중 하나라도 포함하면 그 <p> 통째로 제거.
  //       a) 명시적 placeholder: [태그명], [tag_name]
  //       b) 메타 어휘 + 태그 명: ("사용 가능|허용|사용할 수 있는|금지된?")\s*태그
  //       c) HTML 태그 이름이 본문에 노출: <h[1-6]>, <p>, <ul>, <li>, <strong>, <em>
  //          (단, 단독 단어가 아닌 메타 문맥에서만 — "감싸|사용|출력|포함" 동반 시)
  //       d) 메타 SEO 어휘: "SEO·가독성", "SEO 가독성"
  //       e) IMG 마커 메타: "[IMG_N", "IMG 마커"
  //       f) 메타 형식 어휘: "마크다운/JSON", "코드펜스 금지"
  //
  //     보존: 단순 어휘(예: "h3 검색", "SEO 최적화") 만 있는 경우 통과.
