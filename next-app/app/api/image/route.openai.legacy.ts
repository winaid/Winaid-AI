/**
 * /api/image — OpenAI gpt-image-1.5 이미지 생성 프록시 (next-app, 내부 운영용)
 *
 * 모델: process.env.OPENAI_IMAGE_MODEL || 'gpt-image-1.5'
 *       gpt-image-2 사용 가능하면 OPENAI_IMAGE_MODEL=gpt-image-2 로 env 만 변경.
 ...
 * openai-node 이슈 #1844 로 images.edit 가 gpt-image 계열 거부 중. 픽스되면
 * OPENAI_IMAGE_EDIT_ENABLED=1 로 활성화 가능 (TODO 분기 마련됨).
 *
 * next-app 은 internal admin 도구라 게스트 IP rate limit 미적용 (대시보드 자체에 인증 가드).
 *
 * ⚠️ gpt-image 계열은 organization verification 강제 — 미인증 시 403 → 502 응답.
