 ...
 *
 * 정책 (audit Q-2d, 분기 b2 preview + final): Step 1 (handleExtract — summary/topics
 * gemini ×2) 은 별개 무료 흐름 (/api/gemini 직접). Step 2 만 본 라우트에서
 * useCredit + refund. clinical (Q-2b) 와 동일 패턴.
 *
 * 보안 강화: buildYoutubePrompt 가 server-side 로 이전 — client 가 임의
 * systemInstruction 보내 prompt injection 하던 surface 차단 (Q-2b/c 부수효과 동일).
 *
 * LLM 호출: 내부 /api/gemini server-to-server forward (PRO + googleSearch + 65536 tokens).
 */
...
export async function POST(request: NextRequest) {
  ...
  // 1 user action = 1 credit (audit Q-2d). validation 후 차감.
