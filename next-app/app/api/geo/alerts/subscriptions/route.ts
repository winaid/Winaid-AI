/**
 * /api/geo/alerts/subscriptions — 알림 구독 CRUD (GET / POST)
 *
 * GET ?hospital_name=  → 활성 구독 list 반환
 * POST { hospital_name, our_domains[], threshold_pct?, compare_window_days?, channels, enabled? }
 *   → upsert (hospital_name + channels 조합으로 1구독, MVP — 후속 PR 에서 id 기반 update 추가)
 *
 * SECURITY: next-app — checkAuth (admin_session)
 *           supabaseAdmin (service_role) RLS 우회
 */
...
function validateSubscription(raw: unknown): { ok: true; sub: AlertSubscription } | { ok: false; error: string } {
  ...
  if (!b.channels || typeof b.channels !== 'object') {
    return { ok: false, error: 'channels 객체 필수' };
  }
