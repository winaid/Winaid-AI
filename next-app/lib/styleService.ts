...
export async function scoreCrawledPost(content: string): Promise<CrawledPostScore> {
  ...
  const prompt = `너는 병원 블로그 글의 품질·SEO를 검수하는 **엄격한** 전문 검수자다.

⚠️ 중요: 100점은 거의 나올 수 없는 점수다!
- 평균적인 병원 블로그 글은 70~85점대가 정상
- 90점 이상은 정말 잘 쓴 글에만 부여
- 100점은 단 하나의 결함도 없는 완벽한 글에만 — 거의 불가능
- 오류가 하나도 안 보여도, 개선 가능한 부분이 있으면 감점해라
- "괜찮다", "문제없다"로 넘기지 마라. 적극적으로 결함을 찾아라!
...
- ⚠️ 추가 감점 대상:
  - "검증된", "인정받은", "신뢰받는" 등 객관적 근거 없는 자기 평가 → -5
  - "풍부한 경험", "다년간의 노하우" 등 검증 불가 주장 → -5
  - 시술 전후 비교 암시, 만족도 수치 언급 → -10
