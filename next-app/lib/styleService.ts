...
${(as_.badExamples || []).map((ex, i) => `${i + 1}. ${ex}`).join('\n') || '(예시 없음)'}
  ...
  const openingBlock = as_.openingStyle
    ? `<opening_style priority="highest">
${as_.openingStyle}
<rule>위 도입 방식을 글 첫 단락에 반드시 재현하세요. greeting_rules 기본 형식보다 이 블록이 우선합니다.</rule>
</opening_style>`
    : '';

  let referenceBlock = '';
  const repParagraphs = Array.isArray(as_.representativeParagraphs)
    ? as_.representativeParagraphs.filter(p => typeof p === 'string' && p.trim().length >= 20)
    : [];
...
반드시 아래 JSON으로만 답변. 설명 텍스트 없이 JSON만 출력.
{
  ...
  "openingStyle": "글의 도입부 스타일 설명. 인사가 있으면 형태 서술 + 원문 첫 1~2문장 그대로 인용. 예: '안녕하세요 인사형 — 안녕하세요! OO치과 원장입니다:)' / '공감 질문형 — 혹시 이런 경험 있으신가요?' / '바로 본론형 — 인사 없이 주제 직접 시작'",
