...
export function buildBlogPrompt(req: GenerationRequest): {
  ...
  const systemInstruction = [
    ...
    '',
    '[중복 방지 — 글 전체에서 검사]',
    '- 같은 단어를 2문단 연속으로 사용 금지 (예: "중요합니다"가 연속 문단에 등장하면 실패)',
    '- 같은 표현/구절을 글 전체에서 3회 이상 반복 금지 (예: "~에 도움이 됩니다"를 3번 쓰면 실패)',
    '- 소제목 간 유사한 문장 구조 반복 금지 (매 소제목이 "~경우가 있습니다"로 끝나면 실패)',
    '- 동의어로 바꿔 말하기: 같은 의미를 전달해야 하면 다른 표현을 사용',
    '- 출력 전 자가점검: 각 문단의 마지막 문장 어미가 바로 앞 문단과 다른지 확인',
    '',
    '[의학 표현 정밀도]',
