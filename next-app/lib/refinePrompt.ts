...
// 모든 모드 공통 (의료법, AI냄새, 출력 형식)
const BASE_RULES = `
[⛔ 절대 준수]
1. 의료광고법 준수: "완치", "최고", "보장", "100%", "최소 침습", "무통" 단정 금지
2. 출처/인용 표현 금지: "~에 따르면", "연구에 따르면" → 정보를 자연스럽게 녹여서 서술
3. AI 냄새 제거: "또한", "더불어", "아울러", "이러한", "해당" → 삭제 또는 자연스러운 표현
...
const FORMAL_RULES = `
[격식체 규칙]
- "~요/~죠" 종결어미 금지 → "~입니다/~합니다" 사용
- "~하세요" 행동유도 금지 → "~일 수 있습니다" 가능성 표현`;
 ...
 * ❌ 나쁜 보정: "임플란트는 치아를 대체하는 시술입니다. 잇몸뼈에 인공치근을 식립합니다."  (어투 변경)
 */
const MODE_INSTRUCTIONS: Record<RefineMode, string> = {
  natural: `아래 글을 더 자연스럽고 읽기 편하게 다듬어주세요.

[핵심 원칙 — 가장 중요!]
- 원문의 어투와 말투를 최대한 유지하세요.
- 원문이 "~는데요", "~거든요", "~해요" 체를 쓰고 있다면 그 어투를 그대로 유지하세요.
- 원문이 "~입니다" 체를 쓰고 있다면 그대로 유지하세요.
- 어투를 바꾸는 것이 아니라, 같은 어투 안에서 문장을 매끄럽게 다듬는 것이 목표입니다.

[수정 범위]
- 어색한 문장 연결을 자연스럽게 고치기
- 불필요한 반복 제거
- 너무 긴 문장을 적절히 나누기
- AI 특유의 기계적 표현 → 사람이 쓸 법한 표현으로 (예: "또한" → 삭제, "해당" → "이")

[절대 하지 말 것]
- 원문의 종결어미(~요/~죠/~입니다)를 일괄 변경하지 마세요.
- 원문에 없는 새로운 정보를 추가하지 마세요.
- 원문의 소제목 구조를 바꾸지 마세요.
${BASE_RULES}`,
