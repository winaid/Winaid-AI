...
/**
 * RPC 호출용 클라이언트 — supabaseAdmin (service_role) 우선, 미설정 시 anon.
 * SQL hotfix (2026-05-04_credit_rpc_service_bypass.sql) 가 service_role 우회를
 * 허용하므로 서버 라우트는 supabaseAdmin 으로 호출해야 forbidden 회피.
 */
...
/** 크레딧 1 차감 — Supabase 미설정 시에만 항상 성공 (개발 환경) */
 ...
 *
 * 보안:
 *   - refund_credit RPC 가 caller 검증 (auth.uid() === p_user_id 만 허용,
 *     2026-05-04_credit_refund_rpc.sql 참고)
 *   - amount 1~100 범위만 허용
 *
 * 호출자 흐름:
 *   - 환불 실패는 swallow (호출자 흐름 안 막음 — 운영자가 console.warn 으로 발견)
 *   - 정상 응답 시 remaining 으로 클라이언트 optimistic UI rollback 가능
 */
