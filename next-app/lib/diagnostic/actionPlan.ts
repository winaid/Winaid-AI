...
- 위나이드 블로그 작성 도구를 사용하면 이런 표현이 자동 필터링됩니다.`,
  ...
  // ── Phase 4 — Title 길이 / 키워드 밀도 / 헤딩 계층 / 단락 / HTML 사이즈 / Doctype ──
  [LABELS.title_length]: {
    impact: 'medium', difficulty: 'easy', timeframe: '즉시',
    actionText: '제작사에 "title 태그를 30~60자 범위로 조정 (지역+업종+특화 키워드)" 요청',
    detailedGuide: `이게 뭐예요?
브라우저 탭과 검색 결과에 표시되는 페이지 제목입니다. 너무 짧으면 정보가 부족하고, 너무 길면 검색 결과에서 "..." 으로 잘려서 매력도가 떨어집니다.

어떻게 하나요?
1. 제작사에 다음 문구를 보내세요:
   "각 페이지의 <title> 태그를 30~60자 범위로 조정해주세요. 형식: '{지역명}구 {병원명} - {특화 시술}'"
2. 예시:
   - 30자 미만 (부족): "강남치과"
   - 적정: "강남구 미소치과 - 임플란트·교정 전문" (24자)
   - 60자 초과 (잘림): "서울 강남구 최고의 미소치과 임플란트 교정 보철 모든 진료 가능 24시간 상담"
3. 30분이면 끝납니다. 무료입니다.

팁
- 검색엔진은 보통 50~60자까지만 표시합니다.
- 페이지마다 다른 제목을 사용하세요 (홈/진료안내/의료진소개 등).`,
  },
  [LABELS.keyword_density]: {
    impact: 'medium', difficulty: 'medium', timeframe: '1주',
    actionText: '본문에 핵심 키워드를 자연스럽게 0.5~3% 비율로 분산 배치',
    detailedGuide: `이게 뭐예요?
본문에서 핵심 키워드(예: "임플란트", "강남 치과")가 등장하는 비율입니다. 너무 적으면 검색엔진이 페이지 주제를 모르고, 너무 많으면(5%+) 키워드 스터핑으로 페널티를 받습니다.

어떻게 하나요?
1. 핵심 키워드 1~2개를 정하세요 (예: "강남 임플란트").
2. 본문 100자당 1~2회 정도 자연스럽게 등장하도록 작성:
   - 페이지 첫 단락: 핵심 키워드 포함
   - 중간 H2 헤딩에 키워드 변형 포함
   - 결론 단락에 다시 자연스럽게 포함
3. 같은 단어 반복 대신 동의어·자연 변형 사용 ("임플란트" / "치아 임플란트" / "인공치아").
4. 작성 후 본인 페이지를 읽었을 때 "광고 같다" 느낌이 들면 키워드를 줄이세요.

팁
- AI 작성 도구로 초안 → 원장님 검수가 가장 빠릅니다.
- 위나이드 블로그 작성 도구를 사용하면 적정 밀도로 자동 생성됩니다.`,
  },
  [LABELS.heading_hierarchy]: {
    impact: 'high', difficulty: 'easy', timeframe: '즉시',
    actionText: '제작사에 "헤딩 계층 정리 (H1 1개 → H2 → H3 순서)" 요청',
    detailedGuide: `이게 뭐예요?
H1, H2, H3, H4 는 본문 제목 단계입니다. 책의 "장 → 절 → 항" 같은 구조라서, 검색엔진과 AI 가 페이지의 "어디가 핵심인지" 를 파악하는 기준이 됩니다. H2 없이 H3 부터 쓰면 구조가 어긋나 검색에 불리합니다.

어떻게 하나요?
1. 제작사에 다음 문구를 보내세요:
   "헤딩 계층을 정리해주세요:
   - 페이지당 H1 은 1개만 (페이지 핵심 주제)
   - H2 는 본문의 큰 섹션 제목
   - H3 는 H2 안의 하위 섹션
   - H2 없이 H3 직접 사용 금지"
2. 1~2시간 작업입니다.
...
팁
- 시각적 크기는 CSS 로 조정하면 됩니다 — 헤딩 레벨은 의미(구조) 용입니다.
- 시각장애인 화면 낭독기도 헤딩 순서로 페이지를 탐색하므로 접근성에도 도움됩니다.`,
  },
  [LABELS.paragraph_structure]: {
    impact: 'medium', difficulty: 'easy', timeframe: '즉시',
    actionText: '관리자 모드에서 긴 단락을 50~200자 단위로 짧게 끊어 재구성',
    detailedGuide: `이게 뭐예요?
한 단락이 너무 길면 모바일 화면에서 빽빽한 글자 벽처럼 보여 환자가 그냥 닫고 나갑니다. 50~200자 사이로 끊어주면 가독성이 좋아져 체류 시간이 늘고, AI 도 의미 단위로 인용하기 쉬워집니다.

어떻게 하나요?
1. 관리자 페이지에서 본문이 긴 페이지를 찾으세요 (특히 "원장 인사말", "병원 소개").
2. 각 단락을 의미 단위로 끊어 다음 기준으로 재구성:
   - 1개 단락 = 1개 메시지 (한 가지 주제만)
   - 평균 50~200자 (3~5문장)
   - 단락 사이 빈 줄 추가
3. 모바일에서 미리보기로 확인 — 한 화면에 단락 2~3개가 보이면 적정.

팁
- 핵심 문장은 짧게 끊어 강조 효과를 줄 수 있습니다.
- "이런 분께 추천합니다 / 절차는 이렇습니다 / 비용은" 같이 소제목(H3) 으로 단락을 분리하세요.`,
  },
  [LABELS.html_size]: {
    impact: 'high', difficulty: 'hard', timeframe: '1개월',
    actionText: '제작사에 "이미지 압축(WebP) + 사용 안 하는 CSS/JS 제거" 요청 — HTML 사이즈 줄이기',
    detailedGuide: `이게 뭐예요?
홈페이지 한 페이지의 전체 용량입니다. 200KB 이내가 권장이고, 500KB 가 넘어가면 모바일·느린 인터넷에서 환자가 페이지를 보기 전에 이탈합니다. 구글/네이버도 빠른 페이지를 검색 상위에 올립니다.

어떻게 하나요?
1. 제작사에 다음 문구를 보내세요:
   "메인 페이지 HTML 사이즈가 [현재 KB] 입니다. 다음을 진행해주세요:
   - 이미지 전부 WebP 변환 + 적정 크기로 리사이즈 (헤더 1200×630, 본문 800px 폭)
   - 사용하지 않는 CSS/JavaScript 제거 (DevTools Coverage 탭으로 확인 가능)
   - 큰 슬라이더/배경 영상이 있으면 lazy 로드 또는 제거 검토"
2. 보통 2~4주 작업입니다 (페이지 수 따라 다름).

팁
- pagespeed.web.dev 에 본인 사이트 주소 입력하면 무엇이 무거운지 자동 분석됩니다.
- 한 번에 다 못 줄여도 메인 페이지부터 차근차근 진행하세요.`,
  },
  [LABELS.doctype]: {
    impact: 'low', difficulty: 'easy', timeframe: '즉시',
    actionText: '제작사에 "HTML 첫 줄에 <!DOCTYPE html> 선언 추가" 요청',
    detailedGuide: `이게 뭐예요?
HTML 파일 맨 첫 줄에 들어가는 선언입니다. 없으면 브라우저가 "옛날 모드(quirks mode)" 로 동작해서 레이아웃이 깨지거나 모바일에서 이상하게 보일 수 있습니다.

어떻게 하나요?
1. 제작사에 다음 문구를 보내세요:
   "HTML 파일 가장 첫 줄에 <!DOCTYPE html> 을 추가해주세요. 모든 페이지에 적용 부탁드립니다."
2. 5분이면 끝납니다. 무료입니다.

팁
- 한 줄만 추가하는 일이라 거의 모든 제작사가 즉시 처리합니다.
- 작업 후 본인 사이트를 모바일에서 봤을 때 레이아웃이 정상인지 확인하세요.`,
  },
};
