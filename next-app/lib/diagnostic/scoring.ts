...
// ── URL 분석 헬퍼 — Phase A 6 후보 fix (false positive 박멸) ─────

/** url -> hostname (lowercase). 실패 시 빈 문자열. */
function getHostname(url: string): string {
  try { return new URL(url).hostname.toLowerCase(); } catch { return ''; }
}

/** url -> pathname (lowercase). 실패 시 빈 문자열. */
function getPathname(url: string): string {
  try { return new URL(url).pathname.toLowerCase(); } catch { return ''; }
}

/** internalLinks 중 href 의 pathname 부분이 정규식 매칭 ≥ 1 인지. text 무시. */
function hasInternalPathMatch(crawl: CrawlResult, re: RegExp): boolean {
  return crawl.internalLinks.some(l => re.test(getPathname(l.href)));
}

/** 외부 채널 매칭 — host suffix + 선택적 pathname 접두사. */
interface ChannelMatcher {
  host: string;          // hostname 정확 일치 또는 서브도메인
  pathPrefix?: string;   // pathname 이 이 문자열로 시작해야 매칭 (옵션)
}

/** url 이 channel matcher list 중 어느 하나와 매칭되는지. text 사용 X. */
function matchesChannelMatcher(url: string, matchers: readonly ChannelMatcher[]): boolean {
  let u: URL;
...
/** externalLinks 중 어느 link href 든 매칭되면 true. */
function hasChannelMatch(crawl: CrawlResult, matchers: readonly ChannelMatcher[]): boolean {
  return crawl.externalLinks.some(l => matchesChannelMatcher(l.href, matchers));
}
...
// ── Phase A 후보 1-4: dedicated page path 패턴 ──────────────────

/** 의료진 dedicated page path. /doctor /physician /staff /의료진 /진료진 등. */
const DOCTOR_PAGE_PATH = /\/(doctor|physician|medical[-_]?staff|staff|의료진|진료진|닥터)(\/|$|\?)/i;

/**
 * 본문 의료진 이름 패턴. "원장 홍길동" "대표원장 김철수" 등.
 * 부정 lookahead 로 흔한 비-이름 단어 (인사말/소개/안내/정보 등) 제외.
 */
const DOCTOR_NAME_PATTERN = /(원장|부원장|대표원장|진료원장)\s+(?!인사말|소개|안내|정보|진료|메시지|사진|동영상|약력|이력|경력|학력|자격|프로필|인터뷰|일정|휴진|출근|영상|말씀|글)[가-힣]{2,4}/;

/** 진료/시술 dedicated page path. */
const TREATMENT_PAGE_PATH = /\/(treatment|service|진료|시술|care|clinic|진료안내|진료과목|진료영역)(\/|$|\?)/i;

/** 가격 dedicated page path. */
const PRICING_PAGE_PATH = /\/(price|cost|fee|pricing|요금|진료비|비용)(\/|$|\?)/i;

// ── Phase A 후보 5: 외부 채널 hostname 화이트리스트 ──────────────
// substring 매칭 금지 (예: naver-clinic.com false positive). host suffix + 선택적 path prefix.

const NAVER_CHANNELS: readonly ChannelMatcher[] = [
...
const GOOGLE_CHANNELS: readonly ChannelMatcher[] = [
...
const KAKAO_CHANNELS: readonly ChannelMatcher[] = [
  { host: 'pf.kakao.com' },
  { host: 'kakao.com', pathPrefix: '/_' },
  { host: 'kko.to' },
];

const YOUTUBE_CHANNELS: readonly ChannelMatcher[] = [
  { host: 'youtube.com' },
  { host: 'm.youtube.com' },
  { host: 'youtu.be' },
];

const INSTAGRAM_CHANNELS: readonly ChannelMatcher[] = [
  { host: 'instagram.com' },
];

const ALL_CHANNELS: readonly ChannelMatcher[] = [
  ...NAVER_CHANNELS,
  ...GOOGLE_CHANNELS,
  ...KAKAO_CHANNELS,
  ...YOUTUBE_CHANNELS,
...
function scoreSiteStructure(crawl: CrawlResult): CategoryScore {
  ...
  // ── has_doctor_page 3단계 (Phase A) — dedicated path + 본문 이름 패턴 ──
  // pass: 둘 다 / warning: 하나만 / fail: 둘 다 없음. "원장 인사말" 1단어만 매칭 차단.
  const hasDoctorPath = hasInternalPathMatch(crawl, DOCTOR_PAGE_PATH);
  const hasDoctorName = DOCTOR_NAME_PATTERN.test(crawl.textContent || '');
  if (hasDoctorPath && hasDoctorName) {
    items.push(makeItem(LABELS.has_doctor_page, 20, 20, 'pass', '의료진 dedicated 페이지 + 본문 이름 패턴 감지됨.'));
  } else if (hasDoctorPath || hasDoctorName) {
    items.push(makeItem(LABELS.has_doctor_page, 20, 8, 'warning',
      hasDoctorPath
        ? '의료진 dedicated 페이지는 있으나 본문에 (원장|부원장|대표원장) + 이름 패턴 부족.'
        : '본문에 의료진 이름 패턴은 있으나 dedicated 페이지 경로 없음.'));
  } else {
    items.push(makeItem(LABELS.has_doctor_page, 20, 0, 'fail', '의료진 dedicated 페이지 + 본문 이름 패턴 둘 다 없음.'));
  }

  // ── has_treatment_page 3단계 (Phase A) — dedicated path + 시술 list ──
  // pass: 둘 다 / warning: 하나만 / fail: 둘 다 없음. "진료" 단어 1개 매칭 차단.
  const hasTreatmentPath = hasInternalPathMatch(crawl, TREATMENT_PAGE_PATH);
  const hasServiceList = crawl.detectedServices.length >= 2;
  if (hasTreatmentPath && hasServiceList) {
    items.push(makeItem(LABELS.has_treatment_page, 15, 15, 'pass', `진료 dedicated 페이지 + 시술 ${crawl.detectedServices.length}개 감지.`));
  } else if (hasTreatmentPath || hasServiceList) {
    items.push(makeItem(LABELS.has_treatment_page, 15, 6, 'warning',
      hasTreatmentPath
        ? '진료 dedicated 페이지는 있으나 시술명 2개 미만 — 상세 부족.'
        : `시술명 ${crawl.detectedServices.length}개 감지되나 dedicated 진료 페이지 경로 없음.`));
  } else {
    items.push(makeItem(LABELS.has_treatment_page, 15, 0, 'fail', '진료 dedicated 페이지 + 시술 list 둘 다 없음.'));
  }

  // ── has_service_details 3단계 (Phase A) — 임계값 ≥ 3 ──
  // 기존 ≥ 2 는 비교 글 1문장으로도 false pass. ≥ 3 / ≥ 2 / < 2 로 세분화.
  const svcCount = crawl.detectedServices.length;
  if (svcCount >= 3) {
    items.push(makeItem(LABELS.has_service_details, 10, 10, 'pass', `시술명 ${svcCount}개 감지.`, String(svcCount)));
  } else if (svcCount >= 2) {
    items.push(makeItem(LABELS.has_service_details, 10, 5, 'warning', `시술명 ${svcCount}개 — 3개 이상 권장.`, String(svcCount)));
  } else {
    items.push(makeItem(LABELS.has_service_details, 10, 0, 'fail', `시술명 ${svcCount}개만 감지 — 상세 부족.`, String(svcCount)));
  }
  ...
  // ── has_pricing_page (Phase A) — dedicated path 만 검증 ──
  // 기존 priceRegex 의 "상담" 키워드 + text/href 광범위 매칭은 "상담 신청" 으로 false pass.
  // dedicated 가격 페이지 path 만 인정.
  items.push(hasInternalPathMatch(crawl, PRICING_PAGE_PATH)
    ? makeItem(LABELS.has_pricing_page, 10, 10, 'pass', '비용/가격 dedicated 페이지 감지됨.')
    : makeItem(LABELS.has_pricing_page, 10, 0, 'fail', '비용 dedicated 페이지 경로 없음.'));
...
function scoreContentQuality(crawl: CrawlResult): CategoryScore {
  ...
  if (crawl.totalImages === 0) {
    // unknown 상태는 toCategoryScore 에서 totalMax 집계에서 자동 제외 — 점수 영향 0.
    // 사용자 혼란 방지 위해 detail/rawValue 문구 명확화 (Phase A #6).
    items.push(makeItem(LABELS.alt_ratio, 10, 0, 'unknown', '이미지 0개 — 측정 불가 (점수 집계에서 제외됨).', '-'));
  } else {
...
function scoreExternalChannels(crawl: CrawlResult): CategoryScore {
  ...
  // ── 외부 채널 5종 (Phase A #5) — link href hostname suffix 매칭 ──
  // 기존 substring 매칭 폐기 (naver-clinic.com 같은 false positive 박멸).
  // link text 무시 — 본문에 "youtube.com" 적혀있어도 매칭 X.
  const checks: Array<{ key: LabelKey; points: number; matchers: readonly ChannelMatcher[] }> = [
    { key: 'naver',     points: 12, matchers: NAVER_CHANNELS },
    { key: 'google',    points: 12, matchers: GOOGLE_CHANNELS },
    { key: 'kakao',     points: 10, matchers: KAKAO_CHANNELS },
    { key: 'youtube',   points: 10, matchers: YOUTUBE_CHANNELS },
    { key: 'instagram', points:  8, matchers: INSTAGRAM_CHANNELS },
  ];

  let channelPassCount = 0;
  for (const c of checks) {
    if (hasChannelMatch(crawl, c.matchers)) {
      items.push(makeItem(LABELS[c.key], c.points, c.points, 'pass', `외부 링크 hostname 에서 감지됨.`));
      channelPassCount++;
    } else {
      items.push(makeItem(LABELS[c.key], c.points, 0, 'fail', `외부 링크 hostname 에서 감지 안 됨.`));
    }
  }

  // sameAs (13점) — 동일 hostname suffix 검증 (substring 폐기)
  const sameAsUrls = collectSameAs(crawl.schemaMarkup);
  const sameAsHasChannel = sameAsUrls.some(u => matchesChannelMatcher(u, ALL_CHANNELS));
  items.push(sameAsHasChannel
