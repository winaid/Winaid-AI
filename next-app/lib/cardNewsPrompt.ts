...
export function buildCardNewsPrompt(req: CardNewsRequest): {
  ...
  promptParts.push(
    ...
    '',
    '### 1장: 표지',
    '**제목**: (시선을 멈추게 하는 메인 타이틀, 10자 이내)',
    '**부제**: (부연 설명, 20자 이내)',
    '',
    '### 2장: (슬라이드 역할)',
    '**제목**: (슬라이드 제목, 10자 이내)',
    '**본문**: (핵심 내용 2문장, 각 20자 이내)',
    '',
    '### 3장: (슬라이드 역할)',
    '**제목**: (슬라이드 제목)',
    '**본문**: (핵심 내용 2문장)',
    ...
    '**제목**: (핵심 한 줄 메시지, 10자 이내)',
    `**본문**: (${req.hospitalName ? `"${req.hospitalName}"에서 상담받으세요` : '부드러운 상담 안내 한 문장'})`,
    '',
    '⚠️ 모든 슬라이드에 반드시 **제목**과 **본문**(또는 **부제**) 필드가 있어야 합니다.',
    '⚠️ 마무리 장도 예외 없이 **제목**과 **본문** 형식을 사용하세요.',
    '⚠️ "핵심 메시지", "마무리 문구" 같은 다른 키워드 쓰지 마세요. 반드시 **제목**/**본문**만.',
  );
