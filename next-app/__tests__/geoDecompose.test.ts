/**
 * geoDecompose 회귀 테스트 (next-app) — GEO-1.2 콘텐츠 패턴 분류.
 *
 ...
 *   - next-app: checkAuth (admin_session)
...
(async () => {
  ...
  await test('security: 인증된 public IP 는 통과 시도', async () => {
  ...
  await test('route: POST decompose urls 빈 / 초과 / 형식 검증', () => {
    const p = resolve(REPO_ROOT, 'next-app/app/api/geo/decompose/route.ts');
  ...
  await test('route: next-app — checkAuth (admin_session) 보호', () => {
    const p = resolve(REPO_ROOT, 'next-app/app/api/geo/decompose/route.ts');
    const src = readFileSync(p, 'utf-8');
    assert.ok(/checkAuth\(request\)/.test(src), 'checkAuth 호출 누락');
  });
