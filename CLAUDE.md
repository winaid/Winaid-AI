...
6. **마크다운 syntax** — 출력은 HTML 만. 다음 패턴 모두 금지:
   ...
   1차 책임은 본 룰 준수.
7. **한국어 비문** — LLM 이 자주 만드는 활용 오류·이중 피동 절대 금지:
   - 형용사 동사 활용: `필요하는`, `중요하는`, `안전하는`, `건강하는`, `가능하는`, `충분하는`, `정확하는`,
     `확실하는`, `깨끗하는`, `복잡하는`, `단순하는`, `편안하는`, `신선하는`, `소중하는`, `특별하는`
     → 모두 관형형 `필요한 / 중요한 / ...` 으로 (형용사 "X하다" 는 `-는` 활용 불가)
   - 이중 피동: `되어진다 / 되어지는 / 되어진 / 되어질 / 되어졌`
     → `된다 / 되는 / 된 / 될 / 됐` (이중 피동 "되-" + "-어지-" 중복 금지)
   - 오타: `어떻해` → `어떡해`

   회귀 사례 (2026-05): "필요하는 정보입니다" 같은 비문 응답.
   후처리(`packages/blog-core/src/koreanGrammarFilter.ts`) 가 보수적 치환 (false-positive ≈ 0).
