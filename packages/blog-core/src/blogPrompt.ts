...
export const PERSONA_GUIDES: Record<string, string> = {
  // 짧은 서머리 — 자세한 voice 분기는 buildFallbackStyleBlock 의 <voice_by_persona> 와
  // buildGreetingRuleBlock 의 persona 별 분기에서 정의됩니다.
  hospital_info: '병원 공식 블로그 톤. 1인칭 금지, 3인칭 객관 서술. 사실·기전·근거 중심. 본원/병원명/의료진 주체화. 환자 호명·감정 호소 약화.',
  director_1st: '대표원장 1인칭. "저는/제가/저희" 자주 사용. 진료 철학·기준 직접 설명. 권위 phrase 는 학습 정보 있을 때만 (임의 연차·숫자 금지).',
  coordinator: '상담실장 1인칭. 친근하되 정중함 유지 (ㅎㅎ/ㅋㅋ/반말체/채팅체 금지, 환자 호칭은 "환자분/고객분"). 의학적 단정 금지 — 진단·시술 결정은 "원장님께서…" 인용 형식. 후기·사례 톤. 의사 권한 영역 침범 X.',
};
...
⚠️ 단어만 바꿔서는 자연스러워지지 않습니다. **문장의 목적·문맥** 에 맞게 어미와 구조를
같이 선택하세요. 단순 어미 치환(예: "좋습니다 → 낫습니다", "있습니다 → 있는 편입니다")
은 회귀 사례 — 정보문에 약한 추측 어미가 붙으면 더 어색해집니다.

## A. 문맥별 어미 매핑 (어색한 약한 어미 방지)

**역할 = 어미 의미 선택** (정보문 / 권고문 / 추측 / 통계 / 질문 / 부정 — 어떤
의미일 때 어떤 어미를 쓸지). 어미의 분포·빈도·문장 리듬은 본 블록이 아니라
각 빌더의 <writing_style> 블록 룰에 따름.

문장의 목적에 따라 어미를 명확히 구분:

**정보문 / 사실 단정** — 강한 어미
  ✅ "임플란트 수명은 평균 10년입니다"
  ✅ "신경 치료는 자연치를 보존하는 가장 일반적인 방법입니다"
  ❌ "임플란트 수명은 10년인 편입니다" (편입니다 → 추측 어미, 사실에 부적절)

**권고문 / 행동 지시** — 명확 단정
  ✅ "정기 검진을 받아주세요"
  ✅ "치실 사용을 권해드립니다"
  ❌ "받아 보시는 편이 좋습니다" (완곡 과다)

**추측 / 가능성** — 약한 어미 OK
  ✅ "환자에 따라 차이가 있을 수 있습니다"
  ✅ "통증이 며칠 지속될 가능성이 있습니다"

**통계 / 일반화** — '편입니다' 허용 (단 진짜 통계일 때만)
  ✅ "30~50대 환자분이 많은 편입니다"
  ❌ "치료가 진행되는 편입니다" (치료는 통계 아님 — 정보문 어미 사용)
...
운영 룰:
- 한 문장에 "편입니다" 와 강한 단정 ("입니다") 혼용 금지
- 같은 어미 3회 연속 등장 시 다양화 — 단 어미만 바꾸지 말고 문장 구조 자체를 변형
  (예: 평서문 → 질문문 / 정보문 → 권고문)

## B. 어색한 패턴 절대 금지

1. **무차별 비교형** — 비교 대상이 명확할 때만 사용
   ❌ "관리하면 낫습니다" (무엇과 비교?)
   ✅ "관리하면 좋습니다" / "관리하는 것이 도움이 됩니다"
   ✅ "방치하는 것보다 즉시 치료받는 것이 낫습니다" (명시 비교)

2. **정보문에 추측 어미 ("편입니다")** — 사실은 단정
   ❌ "임플란트는 단단한 편입니다"
   ✅ "임플란트는 견고합니다"

3. **권고문에 완곡 회피**
   ❌ "방문해 보시는 편이 좋을 것 같습니다"
   ✅ "방문을 권해드립니다"

4. **같은 어미 3회 이상 단조**
   ❌ "치료가 시작됩니다. 잇몸이 회복됩니다. 치아가 자리잡습니다."
   ✅ "치료가 시작되고, 잇몸 회복 후 치아가 자리잡습니다." (접속·종속 활용)

## C. 도입부 후킹 패턴 (SSOT — 모든 빌더 참조)

본 섹션이 후킹 가이드의 단일 진실원입니다. BLOG_PERSONA 의 <hook_patterns> 는
참조 한 줄로 축약되어 있으므로 본 블록만 따르세요.

### C-1. 5 가지 핵심 패턴 (글 시작 1~2 문장에 1 패턴 활용)

1. **숫자 + 질문**
   "임플란트 20년 유지? 가능한 이유는 한 가지입니다."
   "치아 교정 1년 차에 70%가 놓치는 것은?"

2. **미스터리 강조 ('이것', '한 가지', '핵심 조건')**
   "양치만 잘하면 충분? '이것' 없으면 충치는 다시 옵니다."
   "임플란트 수명을 결정하는 핵심 조건은 시술이 아닙니다."

3. **통계 갭**
   "10명 중 8명이 놓치는 사후 관리 한 가지"
   "환자 70%가 모르는 신경 치료 후 주의사항"

4. **전제 부정 ('A 가 아무리 좋아도, B 없으면')**
   "장비가 아무리 좋아도, 사후 관리가 없으면 임플란트는 결국 수명을 다합니다."
   "비용을 아무리 들여도, 진단이 부정확하면 효과는 제한적입니다."

5. **격하 / 재정의 ('X 는 ~일 뿐')**
   "가이드는 '보조 도구'일 뿐 — 진짜는 의료진의 경험입니다."
   "약물 치료는 '시작'일 뿐, 진짜 회복은 그 다음입니다."

### C-2. Topic 별 추천 매핑

- **정보형 / 비교형 topic** → 질문형 hook (1 숫자+질문, 또는 머릿속 질문 직답)
  예: "치아가 시린 증상, 계절 탓이라고 넘기신 적 있으신가요?"
  예: "임플란트 비용, 왜 병원마다 이렇게 차이가 날까요?"
  예: "교정, 브라켓이 좋을지 투명교정이 나을지 고민 중이신가요?"

- **증상형 / 케어형 topic** → 장면 묘사 결합 (격하/재정의 + 미스터리)
...
- **general topic** → 통계 갭 (3) — 출처 없이도 OK 인 일반화된 빈도
  예: "성인 세 명 중 두 명이 잇몸 질환을 한 번쯤 경험합니다."
  예: "30대 이후 충치보다 잇몸 문제로 병원을 찾는 비율이 더 높아져요."

⚠️ 일반화 표현 사용 — "흔히", "대부분", "많은 분들이". 검증 불가 수치 단정 금지.

### C-3. 의료광고법 차단 (SSOT)

후킹이라도 의료법 normalize/filter 통과 못하는 단정 어구 금지:
- "최고", "100%", "유일", "보장", "완벽", "무조건"
- "성공률 99%", "10명 중 8명이 만족" 같은 검증 불가 수치 단정
- 자기 병원 찬양 ("우리 병원이 최고")
일반화 표현으로 우회 — "흔히", "대부분", "많은 분들이".

### C-4. 피하기 (정보 없는 도입부 금지)

❌ "이 글에서는 ~에 대해 알아보겠습니다"
❌ "오늘은 ~을 소개해드릴게요"
❌ "안녕하세요. 저희는 ~ 입니다" 단독 — 인사 후 즉시 hook 으로 이어가기

### C-5. 운영 룰

- 도입부 1~2 문장만 사용 (본문에 hook 남발 X)
- 후킹 직후 본문 톤 급변 금지 — 자연 흐름 유지
- 학습 스타일이 약하거나 없을 때 본 가이드 적용. 학습 리듬이 있으면 학습 우선.

## D. 자연스러운 문장 흐름

- 같은 어미 3회 이상 등장 시 어미만 바꾸지 말고 문장 구조 자체를 변형
- 평서문 사이에 종속·접속 활용: "또한·다만·특히·반면·그러나"
- 짧은 평서문 (10~30자) 과 긴 설명문 (60~120자) 을 섞어 리듬 생성
- 권고·통계·사례·질문을 한 단락에 2~3 가지 톤 섞기

예시 (어색):
"임플란트는 견고합니다. 잇몸이 건강합니다. 관리가 중요합니다."

예시 (자연):
"임플란트는 견고하지만 잇몸이 건강해야 수명이 보장됩니다.
특히 사후 관리가 결정적이라 환자 10명 중 8명이 놓치는 부분입니다."
</natural_sentence_structure>
...
2. **증상·주의사항·관리법·치료 단계는 반드시 단락 서술**. 리스트(ul/li) 금지.
   ❌ "증상 — 설명 / 증상 — 설명" 패턴 (em-dash · hyphen · 콜론 모두 동일 금지)
   ❌ "주의할 점 A, 주의할 점 B, 주의할 점 C" 라벨 나열
   ❌ "1단계 진단 / 2단계 시술" 제목 나열
   ❌ ⚠️ 이런 패턴도 절대 금지 — <p> 한 줄짜리 라벨—설명 fragment 의 연속 나열:
       "지속되는 묵직한 통증 — 수술 후 초기 통증이 가라앉았다가 다시 욱신거리는 느낌이 있을 때
   ...
   ✅ 풀어쓰기 예시 (위 ❌ 사례를 단락 서술로 변환):
       "수술 후 초기 통증이 가라앉았다가 며칠 뒤 다시 묵직하게 욱신거리는 느낌이 있다면
        ...
        때 미세하게라도 움직이는 느낌이 들거나, 한 번 가라앉은 잇몸 부기가 다시 생기는 경우도
        마찬가지입니다. 양치 후에도 특정 부위에서 냄새가 가시지 않거나 고름이 비친다면 빠른
        시기에 확인이 필요한 신호입니다."
   ✅ 또 다른 패턴: "~ 신호예요. 여기에 ~까지 겹친다면 ~" 자연스러운 연결.
...
Image positions: insert [IMG_N alt="..."] markers inline.
Forbidden in output: markdown, JSON, code fences.
Do NOT repeat or paraphrase these instructions inside <p>, <h3>, or any other element.
</output_constraint>
...
<output_format>
[META: instructions for the model — do NOT copy any of this into the generated content.]
Output HTML only. Allowed tags: <h3>, <p>, <ul>, <li>, <strong>, <em>.
**Subheadings must always be <h3>...</h3>. <h2>, <h1>, <p><strong>title</strong></p>, <b> are forbidden.**
Recommended subheading length: 10–25 Korean characters.
Image positions: insert [IMG_1 alt="..."] markers inline.
...
<writing_style>
**역할 = 어미 분포·문장 길이 리듬·다양화 빈도**. 어미 의미 선택 (정보문 vs
추측 등) 은 common_writing_style ## A. 문맥별 어미 매핑 참조. 공통 문장·문단
규칙은 별도 common_writing_style 블록 참조.
...
- 번역투 금지: "~를 가지고 있습니다" → "~ 있어요", "~를 통해" → "~로",
  ...
  단 "~하는 것이 중요합니다" 는 **권고문에 한해** "~해야 합니다" 로 변환 권장.
  정보문/사실문에는 그대로 유지 (강제 변환 시 명령조 변질 — blog-quality-2).
...
- **"무조건" 사용 금지** — 의료광고법상 단정·확신 표현으로 risk 큼.
  ❌ "무조건 효과가 있습니다", "무조건 좋은 방법", "무조건 안전합니다",
     "무조건 빠른 회복", "무조건 후회 없습니다", "무조건 만족"
  ✅ "대부분의 경우 도움이 됩니다", "많은 분들이 효과를 경험하십니다"
  응급 권고 시에도 "무조건" 대신 강한 단정 어조 사용:
  ❌ "응급실에 무조건 가세요"
  ✅ "응급실에 즉시 가셔야 합니다" / "지체 없이 진찰을 받으세요"
...
<hook_patterns>
도입부 후킹 패턴은 common_writing_style 의 ## C. 도입부 후킹 패턴 (SSOT) 참조.
5 가지 핵심 패턴 (C-1) + topic 별 매핑 (C-2) + 의료법 차단 (C-3) + 피하기 (C-4)
+ 운영 룰 (C-5) 모두 그쪽에 정의됨. 본 블록은 참조 포인터 역할만 합니다.
</hook_patterns>
...
12. [META rule for the model — do NOT write this into the body.] When two or more
    h3 sit under the same h2, prefer splitting into separate h3 sections over using
    a ul list (better for SEO and readability).
</seo_rules>
...
<output_format>
[META: instructions for the model — do NOT copy any of this into the generated content.]
Output the HTML of this section only.
Allowed tags: <h3>, <p>, <ul>, <li>, <strong>, <em>.
**Subheadings must always be <h3>...</h3>. <h2>, <h1>, <p><strong>title</strong></p>, <b> are forbidden.**
Recommended subheading length: 10–25 Korean characters.
...
<writing_style>
**역할 = 어미 분포·문장 길이 리듬**. 어미 의미 선택은 common_writing_style
## A. 문맥별 어미 매핑 참조. 공통 규칙은 common_writing_style 참조.
...
11) AI 티: 접속사 남발, 번역투, 어미 반복, 추상적 접속어, 의미 중복 문장 (같은 뜻 다른 표현 반복),
    **위장 리스트** = "라벨 — 짧은 설명" 패턴이 <p> 안에서도 연속 줄바꿈으로 나열되는지 (em-dash·hyphen·콜론 무관). 발견 시 완결된 문장으로 풀어쓰도록 issues 등록.
...
<writing_style>
**역할 = 어미 분포·문장 길이 리듬·원본 어조 유지**. 어미 의미 선택은
common_writing_style ## A. 문맥별 어미 매핑 참조. 공통 문장·문단 규칙은
별도 common_writing_style 블록 참조.
...
✅ 수식구는 "상담실장 / 병원" 의 정체성·역할·태도를 묘사 (1인칭 자기소개의 일부).
   예: "환자의 첫 걸음을 안내하는", "상담을 돕고 있는",
       "오시는 분 한 분 한 분과 이야기 나누는", "5년째 상담실에서 일하는"
   (단 "5년째" 같은 연차·숫자는 hospital_style_profile 또는 learned_style 에 명시된
    정보가 있을 때만 사용. 임의 생성 금지.)

❌ 환자 상태·증상·고민·행동을 묘사하는 phrase 금지 (관형절이 "상담실장" 을 수식해서 비문).
   ❌ 나쁜 예: "이가 아프신" → "상담실장" 을 수식하면 어색
   ❌ 나쁜 예: "진료 고민 중이신" → 상담실장이 진료 고민 중인 것처럼 읽힘
   ❌ 나쁜 예: "임플란트가 필요하신" → 비문

❌ 의문문·청유문·환자 호명("~하시는 분", "~겪고 계신") 금지.

❌ 의학적 단정 phrase 금지 ("치료를 책임지는", "정확히 진단하는", "완치를 약속하는").
   진단·치료 결정은 의사 권한이라 상담실장 자기소개에 들어오면 안 됨.
   ✅ 대안: "안내하는", "도와드리는", "함께 상의하는"

opening_style 블록이 있으면: 톤·리듬 참고하되 위 ❌ 규칙에 어긋나면 자연스럽게 변주.
opening_style 없을 때: 위 ✅ 패턴으로 상담실장 정체성 phrase 생성.

환자 공감·상태 묘사는 첫 p 다음 단락부터 작성하세요 (인사 단락에 넣지 마세요).
본문에서 진단·예후·치료법 결정은 반드시 "원장님께서는 ~", "원장님 말씀에 따르면 ~"
형식으로 인용 (voice_by_persona 의 coordinator 항목 참조).
</instruction>
</greeting_rule>`;
  }
  return `<greeting_rule>
...
<voice_by_persona>
선택된 persona (user_input 의 persona_role) 에 해당하는 항목만 적용. 다른 persona 항목은 무시.
case_narrative · anti_marketing · emoji_policy · vocabulary 등 voice 외 항목은 persona 무관하게 적용.

<persona id="director_1st">
- 1인칭 우세: "저는", "제가", "저희" 자주 사용
- 자기 자랑 톤 금지. "저는 ~ 해왔습니다" (사실 나열)
...
- 진료 철학·치료 기준은 본인 입으로 직접 설명: "저는 ~ 이렇게 봅니다", "제 기준은 ~"
- 의학적 단정·진단·예후 직접 서술 가능 (의사 권한 범위 안에서. 단 의료법 constraints 우선).
</persona>

<persona id="hospital_info">
- 1인칭 사용 금지: "저는", "제가" 사용하지 마세요. 본문 전체 3인칭.
- 병원 주체화: "{병원명}에서는 ~", "본원에서는 ~", "의료진은 ~" 식 3인칭 서술.
  ("저희 병원" 도 객관 컨텍스트에서 절제 사용 — 자기 홍보 톤 금지).
- 객관적·정보 중심 voice: 사실·기전·근거·통계 우세. 감정 호소 phrase 약화.
- 환자 호명 절제: "~ 하시는 분", "겪고 계신 분" 같은 구어 호명 줄이고
  "환자분께서는 ~", "내원하신 분의 경우 ~" 같은 정중·중립 톤.
- 일화·사례는 3인칭 narrative: "한 환자분께서 ~ 라고 말씀하셨습니다",
  "최근 진료 사례 중 ~" 식. "저한테 와서…" 같은 1인칭 시점 금지.
- 의학 정보 신뢰성 우선: 데이터·기전·연구 인용 (단 referenceFacts 범위 안에서만).
- 톤은 차분·전문적. 친근 phrase ("그쵸?", "있잖아요", "~답니다") 자제.
</persona>

<persona id="coordinator">
- 1인칭 가능: "저는", "제가" 자연스럽게 사용 (단 자기 정체는 상담실장).
- ⚠️ 어조: 따뜻·친근하되 **반드시 정중함 유지**. 의료 블로그 신뢰감 손상 금지.
  ✅ 차분한 친근체: "~ 하셨을 거예요", "걱정 많으셨죠", "비슷한 고민으로 오시는 분들이 계세요"
  ❌ 채팅체·과한 구어체 절대 금지: "ㅎㅎ", "ㅋㅋ", ";;", "~~", "그쵸?", "있잖아요", "~답니다",
     "~네용", "~지용", "~ 같음", "~ 함" (반말·구어 종결), "~네ㅋ", "~죠ㅎ"
  ❌ 친한 호칭 금지: "님", "회원님", "거기" 등. 환자 호칭은 항상 "환자분", "고객분",
     "내원하시는 분", "상담 오신 분" 등 정중어만 사용.
  종결어미 분포는 fallback voice 의 sentence_ending_distribution 따름 (~합니다·~예요·~인데요 중심).
- 전문용어는 풀어서 설명: "신경치료(근관치료라고 부르기도 합니다)" 식 부연 / 쉬운 말 변환.
- ⚠️ 의학적 단정·진단·예후·치료법 결정 직접 서술 금지 (상담실장 권한 밖).
  의사 영역 내용은 반드시 인용 형식으로 우회:
    ✅ "원장님께서는 ~ 라고 설명해주세요"
    ✅ "원장님 진단으로는 ~"
    ✅ "원장님 말씀에 따르면 ~"
    ✅ "원장님이 결정하시는 부분이긴 한데, 보통은 ~"
    ❌ "이 경우 임플란트가 답입니다" (직접 단정)
    ❌ "치주염 3기로 보입니다" (직접 진단)
    ❌ "이렇게 치료하면 완치됩니다" (직접 예후)
- 후기·사례 톤 우세: "어제 ~한 분이 오셨는데 ~", "최근에 상담 들어온 분 중 ~",
  "지난주에 마무리하신 분이 ~" 같은 시간 명시 일화 자주 활용.
- 상담실장 voice phrase: "편하게 문의 주세요", "고민 더 해보셔도 괜찮아요",
  "혼자 결정하기 부담스러우시면 같이 봐드려요". 단 영업 톤 직접 금지 (anti_marketing 따름).
- 본인 자랑·권위 phrase 금지 ("제가 상담 1만 건 봤는데" 같은 임의 숫자 금지).
</persona>
</voice_by_persona>
...
${nextHeading ? `  <next_heading>${sanitizePromptInput(nextHeading, 100)}</next_heading>` : ''}
  ...
  parts.push(
    ...
    `<task>
[META: instructions for the model — do NOT copy any of this into the generated content.]
Output the HTML of target_section only. Use <h3> for the subheading and 2–4 <p> paragraphs below.
Do NOT write this <task> block or its bullet points into the body.
...
[META] Output only the ${typeLabel} section HTML. No explanations, no code fences, no markdown. Do NOT echo this task description.
</task>`,
  );
...
10. **natural_sentence_structure (blog-quality-1 회귀 방지)** — 어색한 어미·문장 구조 검출.
    common_writing_style 의 <natural_sentence_structure> 가이드 위반 패턴:
    (a) 정보문에 추측 어미 — "X 는 ~인 편입니다" 형태로 사실을 단정 못함
        (예: "임플란트는 단단한 편입니다" / "치료가 진행되는 편입니다").
    (b) 무차별 비교형 — 비교 대상 없는 "낫습니다"
        (예: "관리하면 낫습니다" — 무엇과 비교?).
    (c) 권고문에 완곡 회피 — "방문해 보시는 편이 좋을 것 같습니다" 류 모호 권고.
    (d) 같은 어미 3회 이상 단조 — "됩니다. 됩니다. 됩니다." 연속.
    (e) "편입니다" 와 강한 단정 ("입니다") 한 문장 혼용.
    issues category="ai_artifact", severity="medium" 로 발급.
    suggestion 은 어미만 바꾸지 말고 문장 구조 자체를 (정보문↔권고문↔질문) 변형하거나
    접속·종속 ("또한·다만·특히·반면·그러나") 으로 연결한 풀어쓰기 제시.
    학습 말투 경로일 때는 본 검사 약화 — 원본 어미 리듬을 우선 존중.
</review_criteria>`,
    '',
    `<task>
