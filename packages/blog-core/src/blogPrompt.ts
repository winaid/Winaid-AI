...
마무리 (2문단):
  마무리 형식은 learned_style 또는 medical_blog_voice 의 closing_template 따름.
...
예: fallback 의 1인칭 voice vs persona='hospital_info' — voice 는 conditional 이므로 persona 가 우선.
</priority_order>
...
${keywordInstruction}
${section.type === 'intro' ? `**intro 섹션 추가 지시 (학습본 목차 재현)**: learned_style 의 <table_of_contents> 블록이 있고 비어있지 않으면, 인사/훅/공감 단락 직후 본 intro 섹션의 마지막에 그 목차를 원문 그대로 출력하세요. 번호 리스트 형식·줄바꿈 보존. (위치: intro 끝 = 첫 body section h3 직전. 2-pass 구조라 본 intro 섹션이 출력하지 않으면 목차가 사라짐.) <table_of_contents> 블록이 없거나 빈 문자열이면 목차 생성 금지 — 임의 목차 만들지 마세요.` : ''}
