...
const MEDICAL_LAW_REPLACEMENTS: Array<[RegExp, string]> = [
  ...
  [/반드시\s*효과/g, '효과'],
  [/반드시\s*결과/g, '결과'],
  // "무조건" — 의료광고법상 단정·확신 표현으로 risk 큼. 무차별 차단 복원
  // (blog-quality-2-hotfix, 2026-05-19). PR #249 의 광고 컨텍스트 한정 패턴
  // (자기 병원 / 단정 효과만 매칭) 은 다음 미커버 케이스가 다수 — false positive
  // 보다 더 위험한 false negative 였음:
  //   - "무조건 안전합니다" / "무조건 빠른 회복" / "무조건 후회 없습니다"
  //   - "무조건 OO 병원" / "무조건 좋은 방법" (구체 어휘 변형 무한)
  // 응급 권고 ("응급실에 무조건 가세요") 는 prompt 단 (BLOG_PERSONA
  // natural_compliance) 에서 LLM 이 "즉시·지체 없이" 같은 자연 단정 어조로 회피.
  [/무조건/g, '대부분의 경우'],
