...
// ── XML 이스케이프 (buildStylePrompt — 사용자 유래 문자열이 XML 태그를 깨지 않도록) ──
...
export function buildStylePrompt(
  ...
  // 학습 샘플 텍스트에서 의료광고법 금지어 사전 필터링 (프롬프트 단계 보호)
  // — 후처리(filterOutputArtifacts) 만으로는 LLM 이 학습 구조를 그대로 복제할 때 뚫릴 수 있음
  let medLawFilteredCount = 0;
  const filterText = (s: string | undefined | null): string | undefined => {
    if (!s) return s ?? undefined;
    const { filtered, replacedCount } = filterMedicalLawViolations(s);
    medLawFilteredCount += replacedCount;
    return filtered;
  };
  const filterTextArray = (arr: string[] | undefined): string[] => {
    if (!Array.isArray(arr)) return [];
    return arr.map(s => filterText(s) || '');
  };
...
${safeGoodExamples.map((ex, i) => `${i + 1}. ${escapeXml(ex)}`).join('\n') || '(예시 없음)'}
</good>
<bad label="이 병원답지 않은 문장">
${(as_.badExamples || []).map((ex, i) => `${i + 1}. ${escapeXml(ex)}`).join('\n') || '(예시 없음)'}
</bad>
</style_examples>` : '';
  ...
  const openingBlock = safeOpeningStyle
    ? `<opening_style priority="highest">
${escapeXml(safeOpeningStyle)}

**위 인사문의 톤·길이·줄바꿈 패턴·문장 리듬을 참고하여 본 글 주제에 맞게 자연스럽게 변주하세요.**
- 원문을 한 글자 단위로 그대로 복사하지 마세요 — 인사 형태와 리듬만 유지한 새 인사를 작성합니다.
- 수식구·말투·문장 길이 분포는 원문과 비슷하게.
- 줄바꿈·빈 줄 간격 패턴은 원본 구조 그대로.
- 본 글 주제와 무관한 원문의 고유 명사·시술명은 **포함하지 마세요** (verbatim 누설 차단).
- 이 블록이 greeting_rules 보다 우선.
</opening_style>`
    : '';
...
번호 리스트 형식·빈 줄 간격 보존. 생략 절대 금지.
  ...
  let referenceBlock = '';
  const repParagraphs = safeRepresentativeParagraphs.filter(p => typeof p === 'string' && p.trim().length >= 20);
...
${uniqueSentences.map(s => `<sentence>${escapeXml(s)}</sentence>`).join('\n')}
</original_sentences>`;
    }
  }
