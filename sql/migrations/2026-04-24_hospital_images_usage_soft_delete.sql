...
CREATE OR REPLACE FUNCTION increment_image_usage(image_ids uuid[], owner_id uuid)
RETURNS integer
LANGUAGE sql
SECURITY DEFINER
AS $$
...
REVOKE ALL ON FUNCTION increment_image_usage(uuid[], uuid) FROM public;
GRANT EXECUTE ON FUNCTION increment_image_usage(uuid[], uuid) TO authenticated;
