...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const bgmMood = req.body.bgm_mood || 'calm';
      ...
      for (let i = 0; i < images.length; i++) {
        ...
        execSync(
          `ffmpeg -y -loop 1 -i "${img.path}" -t ${dur} ` +
          ...
          `-c:v libx264 -pix_fmt yuv420p -preset fast -r ${fps} ` +
          `"${segPath}"`,
      ...
      if (transition === 'none' || segments.length <= 1) {
        ...
        for (let i = 1; i < segments.length; i++) {
          ...
          try {
            execSync(
              `ffmpeg -y -i "${videoPath}" -i "${next.path}" ` +
            ...
            console.warn('[card-to-shorts] xfade 실패, concat fallback:', xerr.message);
      ...
      execSync(
        `ffmpeg -y -i "${videoPath}" -f lavfi -i anullsrc=r=44100:cl=stereo -t ${videoDuration} ` +
        `-map 0:v -map 1:a -c:v copy -c:a aac -b:a 128k -shortest "${withAudioPath}"`,
        { timeout: 60000, stdio: 'pipe' },
      );
      ...
      if (narrationFile) {
        ...
        execSync(
          `ffmpeg -y -i "${currentPath}" -i "${narrationFile.path}" ` +
          `-filter_complex "[0:a][1:a]amix=inputs=2:duration=first:dropout_transition=0[out]" ` +
          `-map 0:v -map "[out]" -c:v copy -c:a aac -b:a 128k "${narPath}"`,
          { timeout: 90000, stdio: 'pipe' },
        );
        currentPath = narPath;
      }
      ...
      try { fs.rmSync(workDir, { recursive: true, force: true }); } catch {}
      console.error('[card-to-shorts] Error:', err);
      res.status(500).json({ error: err && err.message ? err.message : '영상 변환 실패' });
    }
  },
);
